...
const sr = require('./songrequest');

let onStatus = () => {};
let onChat = () => {};
let onLog = () => {};            // activity feed for the standalone bot UI
let onSrEvent = () => {};        // song request events -> renderer
let dbOk = false;
...
// Song request events go straight to the renderer via main.js.
sr.setEventHandler((type, data) => { try { onSrEvent(type, data); } catch (e) { console.error('[bot] SR event forward failed:', e); } });
...
function applyConfig(cfg) {
  ...
  sr.configure(cfg.songRequest || {}, currency);
  twitch.configure({
    channel: cfg.channel,
...
module.exports = {
  ...
  setHandlers: (handlers) => {
    ...
    if (handlers.onSrEvent) onSrEvent = handlers.onSrEvent;
  },
  betStatus: () => bets.status(),
  banterControl,        // !banter on/off/now/status hook for commands.js
  sr,                   // song request module (for IPC handlers in main.js)
};
