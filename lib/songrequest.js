...
async function request(login, display, query, flags) {
  ...
  // Notify renderer
  try { onSrEvent('enqueued', { song, queueLength: queue.length }); } catch {}

  const pos = queue.length;
  const costBit = cfg.cost > 0 ? ` (-${cfg.cost} ${CURRENCY})` : '';
...
async function nextSong() {
  ...
  // Re-extract audio URL at play time (URLs expire)
  try {
    const stream = await play.stream(`https://www.youtube.com/watch?v=${song.videoId}`);
    return {
      ...song,
      streamUrl: stream.url,
      streamType: stream.type,  // 'opus', 'webm/opus', etc.
    };
  } catch (e) {
