...
// Cached YouTube client (InnerTube API — same API YouTube's own apps use)
let _ytClient = null;
async function getYt() {
  if (!_ytClient) _ytClient = await Innertube.create();
  return _ytClient;
}
...
async function nextSong() {
  ...
  try {
    ...
    if (!formats.length) {
      currentSong = null;
      return { error: `No audio formats found (id: ${song.videoId})` };
    }
    ...
    const mimeType = (best.mime_type || '').split(';')[0] || 'audio/mp4';
