...
class Twitch extends EventEmitter {
  ...
  connect() {
    this.manualClose = false;
    if (!this.channel) { this.emit('status', 'disconnected', 'No channel set'); return; }
    if (this.ws) { try { this.ws.close(); } catch {} }
    this.emit('status', 'connecting', `Connecting to ${this.channel}…`);
  ...
  _handleLine(line) {
    ...
    if (m001 || rest.includes(' 001 ')) {
      ...
      const mode = this.canWrite()
        ? `writing as ${this.authedAs || this.botUser || 'bot'}`
        : 'read only';
      this.emit('status', 'connected', `Connected to ${this.channel} · ${mode}`);
      this.emit('ready');
      return;
    }
