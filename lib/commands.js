...
function handle(msg, say) {
  ...
  switch (name) {
    ...
    case 'sr': case 'songrequest': case 'request': {
      if (onCooldown('sr', login, flags)) return;
      const query = a.join(' ');
      sr.request(login, display, query, flags)
        .then(msg => say(msg))
        .catch(e => say(`SR error: ${e.message}`));
      return;
    }
    case 'skipsong': case 'nextsong': case 'ns': {
      const msg = sr.skip(login, flags);
      if (msg) say(msg);
      return;
    }
    case 'queue': case 'songqueue': case 'sq': {
      if (onCooldown('queue', login, flags)) return;
      return say(sr.getQueueMsg());
    }
    case 'volume': case 'vol': {
      const msg = sr.setVolume(a[0], flags);
      if (msg) say(msg);
      return;
    }
